...
void vk::SwapchainControl::init()
{
    ...
    vk::SwapchainSupport support = vk::query_swapchain_support(
        backend.instance.physical_device, backend.instance.surface);
    ...
    extent = vk::choose_extent(support.caps, backend.window);
    ...
    sci.surface = backend.instance.surface;
    ...
    if (backend.instance.queues.graphics !=
        backend.instance.queues.present)
    {
        sci.imageSharingMode = VK_SHARING_MODE_CONCURRENT;
        sci.queueFamilyIndexCount = 2;
        sci.pQueueFamilyIndices = backend.instance.queues_indices;
    }
    else
    {
    ...
    VK_CHECK(
        vkCreateSwapchainKHR(backend.instance.device, &sci, nullptr, &swapchain)
    );

    // --- Get swapchain images ---
    uint32_t sc_image_count = 0;
    vkGetSwapchainImagesKHR(backend.instance.device, swapchain, 
...
void vk::SwapchainControl::update_depth_descriptior(const RBDescriptorSet& rb_handle, RBImageHandle value)
{
    ...
    image_info.sampler = backend.get_default_sampler();
    ...
    vkUpdateDescriptorSets(backend.instance.device, 1, &write, 0, nullptr);
}
...
void vk::SwapchainControl::cleanup()
{
    ...
    for (auto iv : image_views)
    {
        ...
        {
            vkDestroyImageView(backend.instance.device, iv, nullptr);
        }
    }
    image_views.clear();
    ...
    {
        vkDestroyImageView(backend.instance.device, depth_image_view, nullptr);
    }

    if (depth_image != VK_NULL_HANDLE)
    {
        vkDestroyImage(backend.instance.device, depth_image, nullptr);
    }

    if (depth_memory != VK_NULL_HANDLE)
    {
        vkFreeMemory(backend.instance.device, depth_memory, nullptr);
    }
    ...
    {
        vkDestroySwapchainKHR(backend.instance.device, swapchain, nullptr);
...
void vk::SwapchainControl::reset_frame_fence(uint32_t frame)
{
    ...
    vkResetFences(backend.instance.device, 1, &f.in_flight);
}
