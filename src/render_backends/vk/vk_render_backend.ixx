...
public:   /// API Section
    ...
    virtual SkinnedMeshGPU create_skinned_mesh(MeshPrimHandle source, const std::vector<SkinVertex>& skin, uint32_t bone_count, const PrimitiveMorphs& morphs, uint32_t morph_count, RTBuildMode rt_build_mode) override;
    virtual RBDeviceAddress upload_bone_matrices(uint32_t instance_id, RBFrameHandle frame, const std::vector<glm::mat4>& matrices, const std::vector<float>& morph_weights) override;
    ...
    virtual void cmd_refit_skinned_blas(RBCommandList cmd, const std::vector<uint32_t>& instance_ids) override;
...
public:   /// Aggregate section. These objects have same lifetime with render backend. use refs
    ...
    vk::BufferManager buffer_manager {instance, instance.device, instance.physical_device, swapchain, immediate_command_pool, debug_object_tracker}; // holds refs
    ...
    vk::PipelineManager pipeline_manager{instance, swapchain, image_manager, buffer_manager, debug_object_tracker};
