...
void reflect::register_object_class_impl(std::string_view name, ObjectFactoryType&& factory, std::optional<JsonSerializer> serializer)
{
    auto data = 
...
const reflect::ObjectFactoryType* reflect::find_actor_factory(std::string_view name)
{
    const auto& registry = get_registry();
    if (registry.contains(name))
    {
        return &registry.at(name).factory;
    }
    return nullptr;
}
